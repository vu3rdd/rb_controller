...
uint64_t calc_dit_time(uint wpm) {
    return 1200/wpm;
}
...
void init_keyer_params(uint wpm) {
    ditTime = calc_dit_time(wpm);
    dahTime = 3 * ditTime;
    sidetone_freq = 800;
    keyDownTime = ditTime;
}

int spawn_keyer(uint wpm) {
    uint64_t t1 = 0;
    bi_decl(bi_program_description("keyer"));
